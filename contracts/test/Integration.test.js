...
describe("SwiftSplit Integration", function () {
  ...
  describe("End-to-End Workflow", function () {
    it("Should handle complete freelance payment workflow", async function () {
      ...
      await usdc.connect(client).approve(swiftSplit.address, ethers.parseUnits("5000", 6));
      await usdc.connect(client).approve(teamSplitter.address, ethers.parseUnits("5000", 6));
      ...
      expect(balance1).to.equal(ethers.parseUnits("1800", 6)); // 1000 + 800
      expect(balance2).to.equal(ethers.parseUnits("700", 6));  // 700
      expect(balance3).to.equal(ethers.parseUnits("500", 6));  // 500
    });
    ...
    it("Should handle payment failures gracefully", async function () {
      ...
      await usdc.connect(client).approve(swiftSplit.address, ethers.parseUnits("500", 6));
